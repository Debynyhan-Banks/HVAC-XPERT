...
Governance, schema validation, private approved-package loading, and deterministic simulator development are authorized. Private extension `RUN-ASXS6-20260802-002` is technically approved and may be composed with base package `RUN-ASXS6-20260802-001` at runtime. The simulator may expose its six phases, explicit component commands, and six measurements, but must not invent or automate transitions because reviewed timing and transition conditions do not exist. Private records must be loaded at runtime and must not be committed. Automated collection is not authorized until a source reviewer sets `approved_for_collection: true` for the exact source. Public export is not authorized while the package legal hold remains active.
